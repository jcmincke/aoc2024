...
do_one total (nbs@(h:t)) = 
  ...
  go '+' current (nbs@(h:t)) = (go '+' (current + h) t) ++ (go '*' (current + h) t) ++ (go '|' (current + h) t)
  go '*' current (nbs@(h:[])) = [current * h]
  go '*' current (nbs@(h:t)) = (go '+' (current * h) t) ++ (go '*' (current * h) t) ++ (go '|' (current * h) t)
  go '|' current (nbs@(h:[])) = [bar2 current h]
  go '|' current (nbs@(h:t)) = 
    let b2 = bar2 current h
    in (go '+' b2 t) ++ (go '*' b2 t) ++ (go '|' b2 t)
  res = (go '+' h t) ++ (go '*' h t) ++ (go '|' h t)
